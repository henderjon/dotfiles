...
//https://shallowsky.com/colormatch/colormatch.php.txt
...
function colorline($r, $g, $b, $name) {
  return "<tr><td>$name <td>($r $g $b) (" . rgb2hex($r, $g, $b) . ")\n<td>"
         . colorswatch($r, $g, $b) . "\n";
}
...
else if (isset($_GET['r']) && isset($_GET['g']) && isset($_GET['b'])) {
  $hex = "";
  $r = $_GET['r'];
  $g = $_GET['g'];
  $b = $_GET['b'];
...
<?php
  if ($r != "" && $g != "" && $b != "") {
    print("\n<hr>\n");
    if ($hex != "") {
      print("<p><big><b>Hex code: </b>#" . $hex . "</big>\n");
    }
    print ("<h2>Matches in rgb.txt:</h2>\n<table>\n");
